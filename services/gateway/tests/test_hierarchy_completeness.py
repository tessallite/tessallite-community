...
class TestLevelsMultiLevel:
    ...
    def test_explicit_hierarchy_has_all_plus_data_levels(self):
        ...
        geo_levels = [r for r in rows if r["HIERARCHY_UNIQUE_NAME"] == "[Hierarchies].[Geography]"]
    ...
    def test_all_level_type_is_one(self):
        ...
        all_level = next(r for r in rows if r["LEVEL_NAME"] == "(All)" and r["HIERARCHY_UNIQUE_NAME"] == "[Hierarchies].[Geography]")
    ...
    def test_time_hierarchy_levels_have_correct_types(self):
        ...
        cal_levels = {r["LEVEL_NAME"]: r for r in rows if r["HIERARCHY_UNIQUE_NAME"] == "[Time].[Calendar]"}
...
class TestMembersParentChild:
    ...
    def test_all_member_exists_with_type_2(self):
        ...
        all_row = next(r for r in rows if r["MEMBER_NAME"] == "All" and r["HIERARCHY_UNIQUE_NAME"] == "[Hierarchies].[Geography]")
    ...
    def test_root_members_have_all_as_parent(self):
        ...
        assert emea["PARENT_UNIQUE_NAME"] == "[Hierarchies].[Geography].[All]"
    ...
    def test_child_members_point_to_parent(self):
        ...
        assert uk["PARENT_UNIQUE_NAME"] == "[Hierarchies].[Geography].[Region].&[EMEA]"
...
class TestTreeOp:
    ...
    def test_self_only_returns_just_the_member(self):
        rows = _rows_members(
            ...
            [_explicit_hierarchy_dim()],
            {
                "MEMBER_UNIQUE_NAME": ["[Hierarchies].[Geography].[All]"],
                "TREE_OP": ["8"],
            },
            _explicit_hierarchy_member_data(),
        )
    ...
    def test_children_of_member_returns_next_level(self):
        rows = _rows_members(
            ...
            [_explicit_hierarchy_dim()],
            {
                "MEMBER_UNIQUE_NAME": ["[Hierarchies].[Geography].[EMEA]"],
                "TREE_OP": ["1"],
            },
            _explicit_hierarchy_member_data(),
        )
...
# REMOVED with Bug-9783. These exercised `_extract_drilldown_member_expansions`,
# which was never called from production code and parsed a `{{...}}` base-set
# shape Excel does not send. Its coverage implied DrilldownMember support that
# did not exist -- the misleading part was the tests, not the absence. The real
# implementation and its 21 tests live in test_bug9638_drilldown_member_collapse.py.
...
class TestFlatHierarchy:
    ...
    def test_single_level_hierarchy_has_all_plus_one_data_level(self):
        ...
        # Bug-6603 groups the flat attribute dim under [Dimensions]; Bug-9771 makes
        # its hierarchy unique name agree with that owning dimension.
        at_levels = [
            r for r in rows
            if r["HIERARCHY_UNIQUE_NAME"] == "[Dimensions].[account_type]"
        ]
    ...
    def test_flat_members_all_have_all_as_parent(self):
        ...
        for m in data_members:
            assert m["PARENT_UNIQUE_NAME"] == "[Dimensions].[account_type].[All]"
...
class TestRaggedHierarchy:
    ...
    def test_ragged_hierarchy_levels_all_present(self):
        ...
        loc_levels = [r["LEVEL_NAME"] for r in rows if r["HIERARCHY_UNIQUE_NAME"] == "[Hierarchies].[Location]"]
    ...
    def test_ragged_leaf_without_intermediate_still_has_parent(self):
        ...
        assert singapore["PARENT_UNIQUE_NAME"] == "[Hierarchies].[Location].[All]"
...
class TestHierarchyMetadataFields:
    ...
    def test_default_member_points_to_all(self):
        rows = _rows_hierarchies("demo", [_explicit_hierarchy_dim()])
        dim_row = next(r for r in rows if r["HIERARCHY_NAME"] == "Geography")
        assert dim_row["DEFAULT_MEMBER"] == "[Hierarchies].[Geography].[All]"
