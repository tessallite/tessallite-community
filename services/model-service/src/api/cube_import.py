"""Cube.dev semantic model import endpoint.

POST /projects/{project_id}/import/cube — accepts Cube YAML or zip,
parses Cube data models and persists them into the target project.
"""
...
class CubeImportResponse(BaseModel):
    models_parsed: int
    models_created: int
...
@router.post(
    "/projects/{project_id}/import/cube",
    response_model=CubeImportResponse,
    tags=["cube-import"],
)
async def import_cube_models(
    ...
    try:
        if filename.endswith(".zip"):
            ...
            parsed = parse_cube_project(files)
        elif filename.endswith((".yml", ".yaml")):
            parsed = parse_cube_yaml(content.decode("utf-8"))
        else:
            raise HTTPException(
                status_code=400,
    ...
    result: MapResult = map_cube_to_tessallite(parsed)
