...
class TestSamlEndpoints:
    ...
    async def test_saml_acs_success(self):
        ...
        with (
            ...
            patch("src.api.sso.process_saml_response", return_value=identity),
...
class TestJitGroupMapping:
    ...
    async def test_jit_adopt_uses_group_role(self):
        ...
        # New call sequence (F-021-03): user lookup, tenant-wide group-role
        # resolution, then the project-scoped binding sync query (empty here,
        # the mapping is tenant-wide).
        empty_bindings = MagicMock()
        empty_bindings.scalars.return_value.all.return_value = []
        db.execute = AsyncMock(side_effect=[user_result, group_result, empty_bindings])
    ...
    async def test_jit_adopt_project_scoped_mapping_creates_binding(self):
        ...
        db.execute = AsyncMock(side_effect=[
            ...
            proj_mappings_result, existing_binding_result,
        ])
