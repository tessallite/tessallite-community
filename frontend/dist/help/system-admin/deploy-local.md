...
| `LICENSE_PUBLIC_KEYS` | When enforcing | — (empty) | Verification public key(s) as `key_id:base64key`. This is **not** built into the software — the setting defaults empty. Obtain the public release key from the Tessallite download/security page (it is public, not a secret) and set it here. Required when `LICENSE_ENFORCEMENT_ENABLED=true`; without it a signed licence cannot be verified. |
