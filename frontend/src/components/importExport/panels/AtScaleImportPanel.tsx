...
export default function AtScaleImportPanel({ projectId }: Props) {
  const t = useT();
  const [file, setFile] = useState<File | null>(null);
  const [result, setResult] = useState<AtScaleImportResponse | null>(null);
  // F-020-03 / G-020-03: preview the loss/warning report before models exist.
  const [preview, setPreview] = useState<AtScaleImportResponse | null>(null);
  const queryClient = useQueryClient();

  const previewMut = useMutation({
    mutationFn: (f: File) => atscaleImportApi.importAtScale(projectId, f, true),
    onSuccess: (data) => setPreview(data),
  });

  const importMut = useMutation({
    mutationFn: (f: File) => atscaleImportApi.importAtScale(projectId, f),
    onSuccess: (data) => {
      setResult(data);
      setPreview(null);
    ...
    return (
      <Box>
        <Alert severity="success" sx={{ mb: 1 }}>
          {t(result.models_created === 1 ? "importDialog.atscaleImportedSingular" : "importDialog.atscaleImportedPlural", { parsed: String(result.models_parsed), ...(result.models_created !== 1 ? { created: String(result.models_created) } : {}) })}
        </Alert>
  ...
  return (
    <Stack spacing={2}>
      <Typography variant="body2" color="text.secondary">
        {t("importDialog.atscaleDescription")}
      </Typography>
      <Box>
        <Button variant="outlined" component="label">
          {file ? file.name : t("importDialog.chooseAtscaleFile")}
          <input
            type="file"
            accept=".zip"
            hidden
