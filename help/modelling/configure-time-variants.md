...
**Period contiguity.** These variants are defined over *consecutive* periods at the grain you chose — a `trailing_3` at month grain means this month and the two months immediately before it, not "the three most recent rows, whenever they happen to be". Tessallite therefore checks that the periods in the frame are adjacent before it returns a value. If a period inside the window carries no data, the window cannot be formed and the result is empty (`NULL`); it is never a total over a wider span quietly relabelled as three months. Reading a Jan + Feb + Apr sum as a trailing-three-month figure is the wrong-number class this guard exists to prevent.
...
This is deliberate fail-closed behaviour: a calendar-period-aware row window uses the same per-grain period key as the `ytd` / `prior_year` family, and a gap inside the frame returns blank instead of a plausible wider number.

**Best practice: make the series dense.** A calendar table has a row for March whether or not anything was sold in March, but that row only reaches the window if the join keeps it. The window counts the periods present in the *result*, and the join decides which rows survive: with the usual fact-preserving orientation (Inner, or Left outer drawn fact-first) a month with no fact rows has no result row, the frame still spans the gap, and the value stays `NULL` even though the calendar table is joined. To make the series dense, declare the calendar join so that the *calendar* side is preserved — Right outer when the join is drawn fact-first, Full outer if you need both sides, or give the join the population role that defines the population (see [Define joins](define-joins.md)) — and query the variant over that calendar dimension at the grain you are reporting. The window then sees January, February, March and April as a continuous run and returns a figure for April, with the empty March contributing nothing. This is the ordinary way to report a sparse series — a product with seasonal gaps, a new region, a measure that only fires on some days — and it is why a date dimension, joined so that its rows survive, is worth building before you add trailing or moving-average variants.
